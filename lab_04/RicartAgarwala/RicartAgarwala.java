...
public class RicartAgarwala {
	...
	public RicartAgarwala(int nodeNum, int seqNum, Driver driverModule) {
		bRequestingCS = false;
	...
	public void receiveRequest(int j, int k) {
		System.out.println("Received request from" + k);
	...
	public void requestTo(int seqNum, int nodeNum, int i) {
		...
		if (i > nodeNum) {
			w[i - 2].println("REPLY, " + seqNum + "," + nodeNum);
		} else {
			w[i - 1].println("REPLY, " + seqNum + "," + nodeNum);
		}
	}
}